...
namespace QuanlyDL.Data
{
    public static class DbHelper
    {
        ...
        public static List<VanBan> LayTatCa()
        {
            ...
            conn.Open();
            using var cmd = conn.CreateCommand();
            // Mặc định: văn bản NHẬP MỚI NHẤT (tạo gần đây nhất) nằm trước
            cmd.CommandText = "SELECT * FROM VanBan ORDER BY Id DESC";
            using var reader = cmd.ExecuteReader();
            ...
            bool sapXepTheoNgayNhanMoiNhat = false)
        {
            ...
            string orderBy = sapXepTheoNgayNhanMoiNhat ? "NgayNhan DESC, Id DESC" : "Id DESC";
        ...
        // ---------------- Cài đặt: tự động tìm kiếm trong Tra cứu ----------------
        // Mặc định BẬT theo yêu cầu.

        private const string KhoaCaiDatTuDongTim = "TuDongTimTraCuu";

        public static bool LayTuDongTimTraCuu()
        {
            var giaTri = LayCaiDat(KhoaCaiDatTuDongTim);
            if (giaTri == null) return true; // chưa cài đặt lần nào -> mặc định BẬT
            return giaTri == "1";
        }

        public static void LuuTuDongTimTraCuu(bool bat)
        {
            LuuCaiDat(KhoaCaiDatTuDongTim, bat ? "1" : "0");
        }
